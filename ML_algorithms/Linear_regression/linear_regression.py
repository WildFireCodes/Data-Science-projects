...
class LinearRegression:
    ...
    def fit(self, X_train, y_train):
        ...
        for _ in range(self.n_iterations):
            self.slope -= self.n_learning_rate * (-2/n) * self.calc_slope(X_train, y_train)
            self.bias -= self.n_learning_rate * (-2/n) * self.calc_bias(X_train, y_train)
    ...
    def calc_slope(self, X, y):
        return np.dot(X.T, (y - self.regression(X)))
